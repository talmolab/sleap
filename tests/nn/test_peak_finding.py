...
def test_find_local_offsets():
    offsets = find_offsets_local_direction(np.array(
         [[0., 1., 0.],
          [1., 3., 2.],
...
def test_find_global_peaks_integral():
    ...
    peaks, peak_vals = find_global_peaks(
        tf.expand_dims(cm, axis=0), threshold=0.1, refinement="integral",
    ...
    peaks, peak_vals = find_global_peaks(
        tf.zeros((1, 8, 8, 3), dtype=tf.float32), threshold=0.1, refinement="integral",
        integral_patch_size=5)
    assert peaks.shape == (1, 3, 2)
    assert peak_vals.shape == (1, 3)
    assert tf.reduce_all(tf.math.is_nan(peaks))
    ...
    peaks, peak_vals = find_global_peaks(
        tf.stack([tf.zeros([12, 12, 3], dtype=tf.float32), cm], axis=0), threshold=0.1,
        refinement="integral", integral_patch_size=5)
    assert peaks.shape == (2, 3, 2)
...
def test_find_global_peaks_local():
    ...
    assert peaks.shape == (1, 3, 2)
    assert peak_vals.shape == (1, 3)
    assert_allclose(peaks[0].numpy(), np.array([[1.75, 2.75], [3.75, 4.75], [5.75, 6.75]]))
    assert_allclose(peak_vals[0].numpy(), [1, 1, 1], atol=0.3)
...
def test_find_local_peaks_rough():
    ...
    cms = tf.stack([cms,
        make_multi_confmaps(instances2, xv=xv, yv=yv, sigma=1.)], axis=0)
...
def test_find_local_peaks_integral():
    ...
    peak_points, peak_vals, peak_sample_inds, peak_channel_inds = find_local_peaks_integral(
        tf.zeros([1, 4, 4, 3], tf.float32), crop_size=5)
    assert peak_points.shape == (0, 2)
    assert peak_vals.shape == (0,)
    assert peak_sample_inds.shape == (0,)
    assert peak_channel_inds.shape == (0,)
...
def test_find_global_peaks_with_offsets(output_stride, min_labels):
    ...
    p += sleap.pipelines.InstanceCentroidFinder(
        center_on_anchor_part=True, anchor_part_names="A", skeletons=min_labels.skeletons
    )
