...
def test_find_global_peaks():
    ...
    peaks, peak_vals = find_global_peaks(cms, threshold=0.1)
...
def test_find_global_peaks_integral():
    ...
    peaks, peak_vals = find_global_peaks_integral(
        tf.expand_dims(cm, axis=0), crop_size=5, threshold=0.1)
...
def test_find_local_peaks():
    ...
    peak_points, peak_vals, peak_sample_inds, peak_channel_inds = find_local_peaks(
        tf.zeros([1, 4, 4, 3], tf.float32), threshold=0.1)
