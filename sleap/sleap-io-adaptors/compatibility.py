...
def patch_labels_class():
    ...
    if not hasattr(SleapIOLabels, 'load_file'):
        SleapIOLabels.load_file = classmethod(
            LabelsAdaptor.load_file
        )
    ...
    if not hasattr(SleapIOLabels, 'from_sleap_labels'):
        SleapIOLabels.from_sleap_labels = classmethod(
            LabelsAdaptor.from_sleap_labels
        )
...
def patch_video_backend_class():
    ...
    if not hasattr(SleapIOVideoBackend, 'get_frame_timestamp'):
        SleapIOVideoBackend.get_frame_timestamp = VideoBackendAdaptor.get_frame_timestamp
    ...
    if not hasattr(SleapIOVideoBackend, 'is_valid_frame_index'):
        SleapIOVideoBackend.is_valid_frame_index = VideoBackendAdaptor.is_valid_frame_index
