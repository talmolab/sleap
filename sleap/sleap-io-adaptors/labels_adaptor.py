...
class LabelsAdaptor:
                    ...
                    if missing[i] and filename:
                        for search_path in new_paths:
                            if os.path.isdir(search_path):
                                potential_path = os.path.join(search_path, os.path.basename(filename))
            ...
            if use_gui and any(missing):
                try:
                    ...
                    if app and app.mainWindow:
                        ...
                        dialog = MissingFilesDialog(filenames, missing, allow_incomplete=True)
                    ...
                    print("Missing files:", [f for f, m in zip(filenames, missing) if m])
    ...
    # Add missing methods that exist in sleap-io but need to be accessible on Labels class
    ...
    @staticmethod
    def from_deepposekit(filename: str, video_path: Optional[str] = None, skeleton_path: Optional[str] = None):
