...
class MultiCheckWidget(QGroupBox):
    """
    A Qt Widget to show multiple checkboxes for selecting from a sequence of numbers.
    ...
    def __init__(self, count, title="", selected=None, default=False, *args, **kwargs):
        ...
        for i in range(count):
            check = QCheckBox("%d"%(i))
            check.stateChanged.connect(lambda e: self.selectionChanged.emit()) # call signal/slot on self            
