...
def generate_training_data(labels, params):
    ...
    points = generate_points(
        labels, params["scale"], frame_limit=params.get("frame_limit", None)
    )
    ...
    if params["instance_crop"]:
        ...
        imgs, points = add_negative_anchor_crops(
            labels, imgs, points, scale=params["scale"]
        )
...
def generate_images(
    ...
    """
    frame_list = [
        (lf.video, lf.frame_idx) for lf in labels.user_labeled_frames[:frame_limit]
    ]
    return generate_images_from_list(labels, frame_list, scale, resize_hack)


def generate_points(
    labels: Labels, scale: float = 1.0, frame_limit: int = None
) -> list:
...
def generate_points_from_list(
    labels: Labels, frame_list: List[Tuple], scale: float = 1.0
) -> list:
    ...
    * augmentation.Augmenter()
    ...
    def lf_points_from_singleton(lf_singleton):
        if len(lf_singleton) == 0:
            return []
        lf = lf_singleton[0]
        points = [inst.points_array * scale for inst in lf.user_instances]
        return points
...
def generate_confmaps_from_points(
    frames_inst_points,
    ...
    confmaps = np.zeros(
        (num_frames, output_size[0], output_size[1], node_count), dtype="float32"
    )
        ...
        for inst_points in points_arrays:
            for node_idx in range(node_count):
                if not np.isnan(np.sum(inst_points[node_idx])):
                    ...
                    _raster_ball(
                        arr=confmaps[frame_idx], ball=ball, c=node_idx, x=x, y=y
                    )

    return confmaps


def generate_pafs_from_points(
    frames_inst_points,
    skeleton,
    shape,
    sigma: float = 5.0,
    ...
    pafs = np.zeros(
        (num_frames, output_size[0], output_size[1], num_channels), dtype="float32"
    )
    for frame_idx, points_arrays in enumerate(frames_inst_points):
        for inst_points in points_arrays:
...
def _raster_ball(arr, ball, c, x, y):
    ...
    if (
        ball_slice_x.stop <= ball_slice_x.start
    ...
    arr[arr_slice_y, arr_slice_x, c] = np.maximum(
        arr[arr_slice_y, arr_slice_x, c], ball[ball_slice_y, ball_slice_x]
    )
...
def _raster_pafs(arr, c, x0, y0, x1, y1, sigma):
    # skip if any nan
    if np.isnan(np.sum((x0, y0, x1, y1))):
        return

    delta_x, delta_y = x1 - x0, y1 - y0

    edge_len = (delta_x ** 2 + delta_y ** 2) ** 0.5
...
def generate_centroid_points(points: list) -> list:
    ...
    centroids = [
        [_centroid(*point_array_bounding_box(point_array)) for point_array in frame]
        for frame in points
    ]
...
def instance_crops(
    imgs: np.ndarray, points: list, min_crop_size: int = 0, negative_samples: int = 0
    ...
    if negative_samples > 0:
        neg_img_idxs, neg_bbs = get_random_negative_samples(
            img_idxs, bbs, img_shape, negative_samples
        )
        neg_imgs, neg_points = _crop_and_transform(imgs, points, neg_img_idxs, neg_bbs)
        crop_imgs, crop_points = _extend_imgs_points(
            crop_imgs, crop_points, neg_imgs, neg_points
        )
...
def _transform_crop_points(points, img_idxs, bbs):
    ...
    crop_points = [
        _translate_points_array(points_array, bbs[i][0], bbs[i][1])
...
def merge_boxes_with_overlap_and_padding(boxes, pad_factor_box, within):
    ...
    if len(merged_boxes) == len(boxes):
        ...
        return merge_boxes_with_overlap_and_padding(
            merged_boxes, pad_factor_box, within
        )
...
def negative_anchor_crops(
    labels: Labels, negative_anchors: Dict["Video", Dict[int, Tuple]], scale, crop_size
) -> Tuple[np.ndarray, List]:
    """
    ...
    frame_list = [(video, frame_idx) for (video, frame_idx, x, y) in neg_anchor_tuples]
    anchors = [[_to_np_point(x, y)] for (video, frame_idx, x, y) in neg_anchor_tuples]
...
def add_negative_anchor_crops(
    labels: Labels, imgs: np.ndarray, points: list, scale: float
) -> Tuple[np.ndarray, List]:
    """Wrapper to build and append negative anchor crops."""
    # Include any *specific* negative samples
    neg_imgs, neg_points = negative_anchor_crops(
        labels, labels.negative_anchors, scale=scale, crop_size=imgs.shape[1]
    )
...
def get_random_negative_samples(img_idxs, bbs, img_shape, negative_samples):
    ...
    for _ in range(max(int(negative_samples * 1.5), negative_samples + 10)):
        ...
        x, y = (
            random.randrange(img_shape[1] - box_side),
            random.randrange(img_shape[0] - box_side),
        )
        sample_bb = (x, y, x + box_side, y + box_side)

        frame_bbs = [
            bbs[i] for i, frame in enumerate(img_idxs) if frame == sample_img_idx
        ]
        area_covered = sum(
            map(lambda bb: box_overlap_area(sample_bb, bb), frame_bbs)
        ) / (box_side ** 2)
...
def _bbs_from_points(points):
    ...
    bbs = [
        point_array_bounding_box(point_array)
        for frame in points
        for point_array in frame
    ]
...
def _crop(imgs, img_idxs, bbs) -> np.ndarray:
    imgs = [
        imgs[img_idxs[i], bb[1] : bb[3], bb[0] : bb[2]] for i, bb in enumerate(bbs)
    ]  # imgs[frame_idx, y0:y1, x0:x1]
    imgs = np.stack(imgs, axis=0)
    return imgs


def fullsize_points_from_crop(
    idx: int, point_array: np.ndarray, bbs: list, img_idxs: list
):
    """
...
def demo_datagen_time():
    ...
    t = timeit.timeit(
        "generate_confidence_maps(labels)", number=timing_reps, globals=globals()
    )
    t /= timing_reps
...
def demo_datagen():
    ...
    imgs, points = generate_training_data(
        labels=labels,
        params=dict(
            scale=scale, instance_crop=True, min_crop_size=0, negative_samples=0
        ),
    )
    ...
    confmaps = generate_confmaps_from_points(
        points, skeleton, img_shape, scale=0.5, sigma=5.0 * scale
    )
    print("--confmaps--")
    ...
    pafs = generate_pafs_from_points(
        points, skeleton, img_shape, scale=0.5, sigma=5.0 * scale
    )
    print("--pafs--")
    print(pafs.shape)
