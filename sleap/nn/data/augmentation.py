...
class AlbumentationsAugmenter:
    ...
    def transform_dataset(self, input_ds: tf.data.Dataset) -> tf.data.Dataset:
        ...
        def py_augment(image, instances):
            ...
            # Augment the image.
            aug_res = self.augmenter(image=image.numpy(), keypoints=[])
            ...
            for i, instance in enumerate(instances):
                ...
                aug_res_i = A.ReplayCompose.replay(
                    aug_res["replay"], image=[], keypoints=instance.numpy()
                )
                aug_instances[i] = aug_res_i["keypoints"]
