...
class Predictor:
    ...
    def run(self, input_video: Union[str, Video], output_path: str):
        ...
        for chunk in range(num_chunks):
            ...
            # Track
            t0 = time()
            tracker.process(mov, frames_idx, instances, skeleton)
            ...
            if chunk % self.save_frequency == 0 or chunk == (num_chunks - 1):
                ...
                save_dict = dict(params=params, skeleton=skeleton,
                                        matched_instances=matched_instances, match_scores=match_scores,
                                        matched_peak_vals=matched_peak_vals, scale=scale,
                                        tracks=tracker.generate_tracks(), track_occupancy=tracker.occupancy(),
                                        )
                ...
                savemat(output_path, save_dict, do_compression=True)
