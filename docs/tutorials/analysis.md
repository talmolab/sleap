...
See `sleap.io.convert` for an explanation of the datasets inside the file.
...
**Note**: The datasets are stored column-major as expected by MATLAB. This means that if you're working with the file in Python you may want to first transpose the datasets so they match the shapes described in `sleap.io.convert`.